...
It seems unlikely that dust extinction is the dominant effect in setting the observed
line ratios.
...
depth differences would result in a substantially ($\approx4\times$) higher 
...
While we cannot rule out the possibility that dust obscuration on sub-beam scales
preferentially blocks our view of some of the high frequency salt emission region, there
is little evidence that the morphology of the emission changes appreciably with frequency.
% However, the morphology of the salt
% lines does not change with frequency.  
Figure \ref{fig:resolutioncomparison}
shows the images from Figure \ref{fig:spatial}, of a salt line in each frequency band,
% selected  salt line from each of the observed frequency windows, as in
% Figure \ref{fig:spatial}, 
convolved to the same resolution.  If selective
obscuration were a significant effect, we might expect to see
progressively less
emission at higher frequencies, or perhaps missing emission at certain
locations at higher frequencies.  There is a gap in the salt emission from the
southwest part of the disk at 1.4 mm that becomes slightly more pronounced at
0.87 mm,
...
Although in this model both SiO and NaCl are released from grains at the
base of the outflow,
% SiO and NaCl have different morphology: 
low-J SiO lines are seen throughout the
