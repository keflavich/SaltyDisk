...
a mean brightness temperature $T_B=230, 220$ and $250$ K in our band 3, 6, and
7 data, respectively.  These values are equivalent to within $\approx10\%$,
which is the approximate calibration uncertainty of ALMA.  If the foreground
cloud had significant optical depth, we would expect the band 7 brightness to
be lower than at band 3; for dust with a typical interstellar medium opacity
index $\beta=1.75$, the dust should be $\approx9\times$ optically thicker at
345 GHz (band 7) than at 100 GHz (band 3).  Since we do not observe a lower
...
is $\tau_{345 \textrm{GHz}} = 2.8$.  If the dust opacity were this large,
the lines would not be detectable in emission.
}
