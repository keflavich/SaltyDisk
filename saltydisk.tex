...
%
% Note that because it averages spectra
...
% with an infinitesimal beam. 
%\ag{This sentence is probably one too many:}
%
...
an unshifted, high signal-to-noise spectrum; one may think of the stacked
spectrum as the output of a matched filter that is optimized for detection
of disk emission.
...
prominent outflow (e.g., SiO and H$_2$O) lines.  Only 2-3 emission lines are now
unidentified in the B6 and B7 spectra, though there are % several
over a dozen in B3 that
we cannot identify. 
...
SLAIM, JPL;][]{Muller2005a,Lovas2005a,Pickett1998a}.  We obtained the
...
$E_U\sim5\ee{4}$ K, and \citet{Cabezas2016a}, which provided more accurate
potential energy and dipole moment functions. The primary result of these
catalogs for the purposes of this work is the prediction of frequencies in
vibrational states higher than those measured in the laboratory. 
...
provided a fortran code to produce those rates.  We have created a
RADEX-compatible molecular data file, which we distribute with this paper, and
used RADEX to examine the excitation.  For T=100 K, $\delta v=1$ \kms \perpc, $X_{NaCl}=10^{-10}$, the
`effective density' at which the lines become detectable is $n\sim10^{6} + 
10^{2v}$ \percc, where $v$ is the vibrational state quantum number.  In other words,
the critical density is two orders of magnitude higher for each progressively
higher vibrational state.  While our collision coefficients do not extend
to $v=6$, if we extrapolate,  densities
$n\gtrsim10^{18}$ \percc are required to collisionally excite the $v=6$ lines.
The declining brightness with rising rotational quantum number rules out
collisional excitation for the vibrational states, as any density high enough
to excite vibrational states would thermalize the rotational levels.
...
The observed emission from vibrationally excited states therefore implies the
...
molecules to still higher states.
% The vibrational emission lines therefore
...
optically thin, so they remain collisionally rather than radiatively excited.
