...
populations may not be well-characterized by a temperature at all.
...
$\Delta$J = 1 or $\Delta$J = 2 transitions are $10^{-10}$~cm$^3$ \pers.  Making the usual
assumption that the collision rates with \hh are similar to those for He, we used
RADEX [ref] to examine the excitation [footnote: the RADEX-compatible molecular data
file incorporating these coefficients is available from ....?].
For T=100 K, $\delta v=1$ \kms \perpc, $X_{NaCl}=10^{-10}$ \rlp{presumably,
the fractional abundance doesn't matter when one is computing critical density;
giving this value will create confusion; the linewidth also is questionable}, the
...
density is two orders of magnitude higher for each progressively
...
$C\approx10^{-5}$ cm$^{3}$ \pers.  Thus, electrons are $10^5\times$ more 
...
the salt lines in such regions.  However, deeper into these disks, only X-rays and cosmic
rays can penetrate to ionize neutrals, resulting in fractional ionizations $X_e < 10^{-6}$,
too small to be relevant. 

The declining brightness with rising rotational quantum number rules out both \hh and electron
...
populations of \hh toward the Orion bar photodissociation region, 
which is well-explained by ultraviolet excitation into higher electronic states,
followed by decay back to excited vibrational levels in the ground electronic
state \citep{Kaplan2017}.  This mechanism avoids the $Delta v=1$ restriction
for infrared excitation.  \hh rotational populations in the ground vibrational
...
There are several problems with this model.  First, we do not detect lines
in higher vibrational states, with weak or ambiguous detections in the v=7 and v=8 
states of NaCl, and nondetections in higher vibrational levels.  Unless there is
...
Second, the binding energy of NaCl and KCl are only a few eV  \rlp{exact values here?}. 
...
are detected.  \sourcei has no associated electron-proton free-free emission
...
it could penentrate to the parts of the disk where
