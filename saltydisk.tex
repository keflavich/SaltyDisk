...
% when being ejected from dying stars before being
...
%
%, and for the lines discussed here, includes no emission associated with the outflow.
% rlp: I took the last phrase out because the stacked spectrum does show some lines
% that are associated with the outflow, like SiO and H2O
%
...
excitation patterns.
\rlp{The RADEX models show this??  If the radiation field is sufficient to
excite the high vibrational levels, why does it not also thermalize the
rotational levels to a very high temperature?}
...
there is a zone where either dust has not  formed 
\rlp{dust has not formed?? that is applicable to evolved stars, but not
to a protstar like SrcI}
or
...
outflow \citep{Hirota2017a}. We evaluate three scenarios for dust destruction:
sputtering by high-energy particles, gas-phase formation from atomic gas, and
thermal desorption.
\rlp{gas phase formation from atomic gas is not a dust destruction scenario.
Presumably you mean release of Na, K, and Cl from grains, which must then
form NaCl and KCl in the gas phase.}
